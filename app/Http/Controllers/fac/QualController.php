...
class QualController extends Controller
{
    public function index()
    {
		...
		$qualifications = DB::table('qual')->orderBy('year','DESC')->get()->where('faculty_id', '=', Auth::user()->username);
	...
	public function update(Request1 $request){
		...
		DB::table('qual')->where('id', $qid)->update(["course"=>$course, "institute"=>$institute, "year"=>$year, "cgpa"=>$cgpa]);
	...
	public function admin()
	{
		if(Auth::user()->hasRole('admin')){
            return view('facul.adminindex');
        }
	}
	
	public function newf()
	{
		return view('facul.new1');
	}
